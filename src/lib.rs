...
use axum::body::Body;
use axum::http::{Request, Response, StatusCode, HeaderValue, Method};
use bytes::Bytes;
...
pub fn make_router(
    ...
    Router::new()
        .route(
            ...
            .layer(middleware::from_fn_with_state(api_key.clone(), require_auth)),
        )
...
async fn require_auth(State(api_key): State<String>, req: Request<Body>, next: Next) -> Result<Response<Body>, StatusCode> {
...
pub async fn proxy_with_upstream(req: Request<Body>, _vault: Arc<PiiVault>, upstream_url: String) -> Response<Body> {
    ...
    let whole = match axum::body::to_bytes(req.into_body(), 10_485_760).await {
        Ok(b) => b,
        Err(_) => return Response::builder().status(StatusCode::BAD_REQUEST).body(Body::from("failed to read body")).unwrap(),
    };
