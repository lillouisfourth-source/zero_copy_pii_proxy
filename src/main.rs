...
async fn main() {
    ...
    let proxy_port = std::env::var("PROXY_PORT").ok().and_then(|s| s.parse::<u16>().ok()).unwrap_or(3000u16);
    let upstream_url = std::env::var("UPSTREAM_API_URL").unwrap_or_else(|_| "https://api.openai.com/v1/chat/completions".to_string());

    // Parse PII_PATTERNS into a Vec<String> and prepare replacements
    let patterns_env = std::env::var("PII_PATTERNS").unwrap_or_else(|_| "password,secret,ssn".to_string());
    let patterns_owned: Vec<String> = patterns_env.split(',')
    ...
    let replacements_owned: Vec<String> = patterns_owned.iter().map(|_| "[REDACTED]".to_string()).collect();
    ...
    let api_key = std::env::var("PROXY_API_KEY").unwrap_or_else(|_| "change_me_in_production".to_string());

    let app = make_router(vault.clone(), prometheus_handle.clone(), api_key.clone(), upstream_url.clone());
